...
exports.applyJob = exports.getSelectedProfile = exports.getUserJobCount = exports.getFilteredJobs = exports.getJobProfile = exports.jobPostDetails = void 0;
...
const jobPostDetails = async (req, res) => {
    try {
        ...
        const newJob = new jobDetails_1.jobProfile({
            role,
            company,
            experience,
            mailId,
            ...
            skills,
            Responsibilities,
            postedBy: new mongoose_1.default.Types.ObjectId(postedBy),
        });
        await newJob.save();
...
exports.jobPostDetails = jobPostDetails;
...
const getJobProfile = async (req, res) => {
    try {
        ...
        const jobs = await jobDetails_1.jobProfile.find(filter).sort({ _id: -1 });
        return res
            .status(200)
            .json({ message: "fetched jobs", total: jobs.length, jobs });
    }
...
const getFilteredJobs = async (req, res) => {
    try {
        ...
        const totalJobsPosted = await jobDetails_1.jobProfile.countDocuments({
            postedBy: objectUserId,
        });
        const jobsPosted = await jobDetails_1.jobProfile
        ...
        const counts = await jobStatus_1.applicationStatus.aggregate([
...
const getUserJobCount = async (req, res) => {
    try {
        let { userId } = req.body;
        if (!mongoose_1.default.Types.ObjectId.isValid(userId)) {
            return res.status(400).json({ error: "Invalid userId" });
        ...
        const objectUserId = new mongoose_1.default.Types.ObjectId(userId);
        const jobsApplied = await jobStatus_1.applicationStatus
...
exports.getUserJobCount = getUserJobCount;
const getSelectedProfile = async (req, res) => {
    try {
        let { selectedId } = req.query;
        const selectedProfile = await jobDetails_1.jobProfile.findById(selectedId);
        return res
            .status(200)
...
exports.getSelectedProfile = getSelectedProfile;
const applyJob = async (req, res) => {
    try {
        let { jobProfile_Id, user_Id, userName, mobile, firstName, lastName, postedBy, } = req.body;
        ...
        const appStatus = new jobStatus_1.applicationStatus({
            mailID: userName,
            user_Id: user_Id,
            jobProfile_Id: jobProfile_Id,
            jobStatus: "Applied",
