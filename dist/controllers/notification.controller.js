...
exports.verifyNotifications = exports.getNotificationByAction = exports.getNotifications = void 0;
...
const getNotifications = async (req, res) => {
    try {
        ...
        // });
        const notificationresult = await notification_1.notification
            .find({
            receiver_user_id,
...
//   }
// };
const getNotificationByAction = async (req, res) => {
    try {
        let { triggeredBy, receiver_user_id, role, jobProfile_Id, type, message } = req.body;
        ...
        const job = await jobDetails_1.jobProfile.findById(jobProfile_Id);
        if (!job)
            return res.status(404).json({ error: "Job not found" });
        let finalReceiverId;
        ...
        else if (["Rejected", "InterviewScheduled", "Selected", "OfferSent"].includes(type)) {
            if (!receiver_user_id) {
                return res.status(400).json({
                    error: "receiver_user_id is required for admin-triggered actions (Rejected, InterviewScheduled, etc.)",
                });
            }
        ...
        const jobApplication = await jobStatus_1.applicationStatus.findOne({
            jobProfile_Id,
...
exports.getNotificationByAction = getNotificationByAction;
const verifyNotifications = async (req, res) => {
    try {
        ...
        const updatedNotification = await notification_1.notification.findByIdAndUpdate(notificationId, { isRead: true }, { new: true });
